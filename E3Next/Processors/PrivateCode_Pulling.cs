...
namespace E3Core.Processors
{
    public static class SuperMode
    {
        ...
        private static void CheckAndCamp()
        {
            ...
            {
                CampUpdateTimer.SetTime(0, 10, 10, 0);
                string timeLeft = PrivateCode.CampOutTimer.TimeLeftString(); // Assuming that the time span you're interested in is 10 seconds
                MQ.Write("\a-yCamp Time Left " + timeLeft + " " + PrivateCode.CampOutTimer.IsElapsed().ToString());
            }
            // Camp out 
            if (MQ.Query<int>($"${{Me.XTarget[1].ID}}") == 0 && PrivateCode.CampOutTimer.IsElapsed())
            {
                MQ.Write("Starting To Camp 90s");
                MQ.Cmd("/beep");
                MQ.Cmd("/beep");
                ElapsedTimer PauseTimer = new ElapsedTimer();
                PauseTimer.SetTime(0, 0, 30, 500);
                while (!PauseTimer.IsElapsed()) {
                    EventProcessor.ProcessEventsInQueues("/pullingmode");
                    if (MQ.Query<int>($"${{Me.XTarget[1].ID}}") > 0 || !PrivateCode.CampOutTimer.IsElapsed()) return;
                }
                if (MQ.Query<int>($"${{Me.XTarget[1].ID}}") > 0 || !PrivateCode.CampOutTimer.IsElapsed()) return;

                MQ.Write("Starting To Camp 30s");
                MQ.Cmd("/beep");
                MQ.Cmd("/beep");
                PauseTimer.SetTime(0, 0, 30, 500);
                while (!PauseTimer.IsElapsed()) {
            ...
            EventProcessor.RegisterCommand("/pullingmode", (x) =>
            {
                if (x.args.Count > 0 && x.args[0].Equals("on", StringComparison.OrdinalIgnoreCase))
                {
                    if (x.args.Count > 1 && MQ.Query<int>($"${{Spawn[pc {x.args[1]}].ID}}") > 0)
                    {
                        ...
                        PrivateCode.CampOutTimer.SetTime(1, 33, 30, 130);
